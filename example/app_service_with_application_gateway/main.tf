...
module "Azure_App_Service" {
 ...
 #if you want to restrict public access so set public_network_access_enabled = false
...
#    if not then this module will create a application insights by providing a application insights name
  ...
  # set create_application_gateway  = true to create application gateway that include app service in this backend pool
    create_application_gateway  = true
    app_gateway_subnet_id       = module.vnet_main.subnet_ids["app_gateway_subnet"]
} 